...
// IMPORTAMOS EL MAPA DE VISUALIZACIÓN (Asegúrate de tener este archivo en components)
...
function Visitas() {
    ...
    const [tableroSupervisor, setTableroSupervisor] = useState([]); // Tabla global
    
    // Listas para Dropdowns
    const [listaClientes, setListaClientes] = useState([]);
    const [listaTecnicos, setListaTecnicos] = useState([]);

    // Estados del Formulario Supervisor
    ...
    // Estados para el Mapa de Check-in
    ...
    // --- TUS URLS DE RAILWAY ---
    const URL_VISITAS = 'https://visitas-19-26373.up.railway.app';
    const URL_CLIENTES = 'https://clientes-19-26373.up.railway.app';
    const URL_USUARIOS = 'https://usuarios-19-26373.up.railway.app';
    ...
    const cargarTableroSupervisor = async (token) => {
        try {
            const res = await axios.get(`${URL_VISITAS}/api/tablero-supervisor`, { 
                headers: { Authorization: `Bearer ${token}` } 
            });
            setTableroSupervisor(res.data);
        } catch (error) { console.error("Error cargando tablero"); }
    };

    const cargarListasParaFormulario = async (token) => {
        try {
            // Clientes
            const resC = await axios.get(`${URL_CLIENTES}/api/clientes`, { 
                headers: { Authorization: `Bearer ${token}` } 
            });
            setListaClientes(resC.data);

            // Usuarios (Filtramos técnicos)
            const resU = await axios.get(`${URL_USUARIOS}/api/usuarios`);
            setListaTecnicos(resU.data.filter(u => u.id_rol == 3));
        } catch (error) { console.error("Error cargando listas"); }
    };

    const cargarMisVisitas = async (token) => {
        try {
            const res = await axios.get(`${URL_VISITAS}/api/mis-visitas`, { 
                headers: { Authorization: `Bearer ${token}` } 
            });
            setVisitas(res.data);
        } catch (error) { console.error("Error cargando agenda"); }
    };

    // --- FUNCIONES DE MAPA Y CHECK-IN ---
    const iniciarCheckIn = (id) => {
        if (!navigator.geolocation) {
            alert("Tu navegador no tiene soporte GPS.");
            return;
        }

        setIdVisitaCheckIn(id);
    ...
    const confirmarCheckInFinal = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.put(`${URL_VISITAS}/api/visitas/${idVisitaCheckIn}/checkin`, 
            ...
            console.error(error);
            alert('Error al registrar el Check-in en el servidor.');
        }
    };

    // --- FUNCIONES DE GESTIÓN (Checkout y Planificar) ---
    const handleCheckOut = async (e) => {
        e.preventDefault();
        try {
            const token = localStorage.getItem('token');
            await axios.put(`${URL_VISITAS}/api/visitas/${visitaActiva}/checkout`, 
            ...
            setReporte('');
            cargarMisVisitas(token);
        } catch (error) { alert('Error al finalizar la visita.'); }
    };
    ...
    const handlePlanificar = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post(`${URL_VISITAS}/api/visitas`, 
            ...
            setFecha('');
            cargarTableroSupervisor(token);
        } catch (error) { alert('Error al planificar la visita.'); }
    };
    ...
    if (!usuario) return <div className="p-5 text-center">Cargando...</div>;
            ...
            {buscandoUbicacion && (
                <div className="position-fixed top-0 start-0 w-100 h-100 d-flex justify-content-center align-items-center bg-dark bg-opacity-75" style={{ zIndex: 2000 }}>
                    <div className="text-center text-white">
                        ...
                        <h4 className="mt-3">🛰️ Localizando...</h4>
                ...
                <div className="position-fixed top-0 start-0 w-100 h-100 d-flex justify-content-center align-items-center bg-dark bg-opacity-50" style={{ zIndex: 1050 }}>
                    <div className="card shadow-lg" style={{ width: '90%', maxWidth: '500px' }}>
                        ...
                        <div className="card-body p-0">
                            {/* Componente de Mapa OSM */}
                            <MapaUbicacionOSM 
                                latitud={ubicacionDetectada.lat}
                                longitud={ubicacionDetectada.lon}
                            />
                            <div className="p-3 text-center">
                                <p className="small text-muted">Coordenadas: {ubicacionDetectada.lat}, {ubicacionDetectada.lon}</p>
                            ...
                            <div className="row">
                                <div className="col-md-4 mb-3">
                                    ...
                                    <select className="form-select" value={idCliente} onChange={e => setIdCliente(e.target.value)} required>
                                        ...
                                        {listaClientes.map(c => (
                                            <option key={c.id_cliente} value={c.id_cliente}>{c.nombre_empresa}</option>
                                        ))}
                                    </select>
                                </div>
                                <div className="col-md-4 mb-3">
                            ...
                            <div className="table-responsive">
                                <table className="table table-hover align-middle">
                                    ...
                                    <tbody>
                                        ...
                                        {tableroSupervisor.length === 0 && <tr><td colSpan="6" className="text-center">No hay visitas registradas.</td></tr>}
                                    </tbody>
                                </table>
                            </div>
                        </div>
            ...
            {usuario.id_rol == 3 && (
                <div>
                    ...
                    <div className="row">
                        ...
                        {visitas.length === 0 && <p className="text-muted">No tienes visitas asignadas.</p>}
                    </div>
                </div>
            )}
        </div>
